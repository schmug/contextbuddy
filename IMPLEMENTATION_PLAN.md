...
| Hook recursion guard | `CONTEXTBUDDY_SKIP=1` env var is checked at the top of both hooks. Load-bearing since issue #13: `claude -p` (non-bare) runs hooks, so `grader/invoke.sh` sets it on the child. |
