...
- `dominant_signal` — set to **one of `"confidence"`, `"atomicity"`, `"drift"`, `"pollution"`** when a single dimension's threshold cross is the reason a state would change, OR `null` if no dimension drove a state change. **Never** emit `"loop"`, `"context_pressure"`, or `"harm"` — those sentinels are set mechanically by the plugin (`harm` only by the typesafe backend, from `signals.destructive` / `signals.bypass`) and will overwrite or clear your value when applicable. When multiple dimensions cross thresholds, use this precedence: `atomicity > confidence > drift > pollution`.
