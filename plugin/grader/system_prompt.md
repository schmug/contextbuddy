...
- `tokens_used`, `tokens_limit` — copy from input. The plugin overwrites both with the values it measured from the transcript whenever a transcript is available and the measured count is within the limit; when the count exceeds the limit (a model with a larger context window), your copy stands.
